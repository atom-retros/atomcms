<div class="select-none" x-data='reactions(@json($myReactions), @json($articleReactions), "{{ route('article.reaction.add', $article->slug) }}")'>
    <div class="flex w-full flex-wrap gap-2 mt-4 bg-gray-100 dark:bg-gray-900 rounded-lg p-2">
        <template x-if="isAuthenticated">
            <div class="px-2 hover:scale-110 transition-all font-semibold h-8 flex items-center justify-center border-2 text-xs border-yellow-400 cursor-pointer bg-[#eeb425] text-white rounded-lg" data-modal-toggle="article-reactions-modal">
                {{ __('Add') }}
            </div>
        </template>
        <template x-for="articleReaction in articleReactions">
            <div class="w-12 h-8 border-2 border-gray-300 dark:border-gray-800 dark:hover:bg-gray-700 rounded-lg flex gap-2 font-bold text-sm justify-center items-center"
                :class="{ 'bg-gray-300 dark:bg-gray-800 dark:border-gray-700': userHasReaction(articleReaction), 'cursor-pointer hover:bg-gray-200 hover:scale-110 transition-all': isAuthenticated }"
