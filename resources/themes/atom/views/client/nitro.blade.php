...
<div class="absolute top-4 left-4 flex gap-x-2" style="z-index: 10;">
    <a href="{{ route('me.show') }}" class="client-buttons text-center rounded py-1 px-2 text-sm text-white">
    ...
    <button onclick="window.location.href = window.location" class="client-buttons text-center rounded py-1 px-2 text-sm text-white">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
        </svg>
    </button>

    <button onclick="toggleFullscreen()" class="client-buttons text-center rounded py-1 px-2 text-sm text-white">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15" />
        </svg>
    </button>

    <div class="client-buttons flex items-center justify-center gap-x-1 rounded py-1 px-2 text-sm text-white">
...
<script>
    ...
    const fetchInitialCount = setTimeout(function () {
        getOnlineUserCount();
    }, 4000);
    ...
    function getOnlineUserCount() {
        fetch('{{ route('api.online-count') }}')
            .then(function (response) {
                console.log("we hit counter");
                return response.json();
            })
            .then(function (response) {
                clearTimeout(fetchInitialCount);
