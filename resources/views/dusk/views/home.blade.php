<x-app-layout>
            ...
            <a href="{{ route('profiles', Auth::user()) }}"
               class="absolute bottom-2 -left-4 lg:left-8 drop-shadow transition duration-300 ease-in-out hover:scale-105">
                <img style="image-rendering: pixelated;"
                     src="{{ setting('avatar_imager') }}{{ Auth::user()->look }}&direction=2&head_direction=3&gesture=sml&action=wav&size=l"
                     alt="">
            </a>
        </div>
    ...
    <div class="col-span-12 md:col-span-8">
        <x-layout.content-card icon="hotel-icon">
            <x-slot:title>
                {{ sprintf(__('User Referrals (%s/%s)'), Auth::user()->userReferral?->referrals_total ?? 0, setting('referrals_needed')) }}
            </x-slot:title>
            ...
            <div class="text-sm">
                ...
                @if (Auth::user()->userReferral?->referrals_total >= (int) setting('referrals_needed'))
                ...
                @else
                    <button disabled class="mt-2 w-full rounded bg-[#171a23] p-2 text-white">
                        {{ sprintf(__('You need to refer :needed more users, before being able to claim your reward', ['needed' =>Auth::user()->referralsNeeded() ?? 0]), Auth::user()->userReferral?->referrals_total ?? 0) }}
                    </button>
                @endif
            </div>
        </x-layout.content-card>
    </div>
