...
use App\Nova\Repeater\Badge;
use App\Nova\Repeater\Furniture;
...
class WebsiteShopArticle extends Resource
{
    /**
     * The model the resource corresponds to.
     *
     * @var class-string<\App\Models\WebsiteShopArticle>
     */
    public static $model = \Atom\Core\Models\WebsiteShopArticle::class;
    ...
    {
        return [
            BelongsTo::make('Category', 'category', WebsiteShopCategory::class)
                ->sortable()
                ->rules('required'),
            ...
            Number::make('Rank Term (months)', 'rank_term')
                ->hideFromIndex()
                ->nullable()
                ->rules('sometimes'),
            ...
            Number::make('Diamonds')
                ->rules('required')
                ->default(0),

            Repeater::make('Badges', 'badges')
                ->repeatables([Badge::make()])
                ->uniqueField('code')
                ->asJson(),

            Repeater::make('Furniture', 'furniture')
                ->repeatables([Furniture::make()])
                ->asJson(),
